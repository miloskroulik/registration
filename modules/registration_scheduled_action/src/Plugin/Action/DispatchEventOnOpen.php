 ...
 *   label = @Translation("Dispatch registration open event"),
 *   type = "registration",
 * )
 */
class DispatchEventOnOpen extends ConfigurableActionBase implements ContainerFactoryPluginInterface, QueryableActionInterface {
  ...
  /**
   * DispatchEventOnOpen constructor.
   *
   ...
   * {@inheritdoc}
   */
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition): DispatchEventOnOpen {
    return new static(
      $configuration,
      $plugin_id,
      $plugin_definition,
  ...
  public function execute($object = NULL) {
    if ($object && ($host_entity = $this->getHostEntity($object))) {
      ...
      $this->dispatcher->dispatch($event, RegistrationEvents::REGISTRATION_SETTINGS_OPEN);
    }
  }
  ...
  public function getAllowedPositions(): array {
    // The event can be dispatched before or after the settings open date.
    return ['before', 'after'];
  }
  ...
  public function getDateFieldLabel(): string {
    return $this->t('Registration settings open date');
  }
  ...
  public function getKeyValueStoreCollectionName(): string {
    return 'registration_scheduled_action.registration_settings_open';
  }
  ...
  public function getQuery(ScheduledActionInterface $scheduled_action): SelectInterface {
    ...
    $query->condition('r.open', $date_times, 'BETWEEN');

...
class RegistrationSettingsForm extends FormBase {
  ...
  protected function getRegistrationSetting(string $key): mixed {
    // First check the settings entity.
    $entity = $this->getEntity();
    if ($entity->hasField($key) && !$entity->get($key)->isEmpty()) {
      // Entity has the setting, return its value.
      $setting = $entity->get($key)->first()->getValue();
    ...
    if (!$entity->hasField($key)) {
      if (!$entity->get('settings')->isEmpty()) {
        $settings = $entity->get('settings')->first()->getValue();
        if (!empty($settings)) {
          $settings = unserialize($settings['value']);
          if (!empty($settings[$key])) {
            // Entity has the additional setting, return its value.
            return $settings[$key];
          }
        }
      }
    ...
    // The entity does not have the setting yet.
    // Get a default value from the host entity registration field defaults.
    $field_definition = $this->registration->getRegistrationField($this->getHostEntity());
